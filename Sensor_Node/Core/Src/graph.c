 ...
 * Modified by Ankita B. 
 * 
 * Adjacency list graph implementation and helper functions (some funcitons adapted from 15-641 Lab1)
 */
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
void print_adj_list(adj_node_t* adj){
    ...
    while(curr != NULL){
        sprintf(printf_buf,"%u ", curr->addr);
        UART2_tx(printf_buf);
...
void print_graph(graph_t *net_graph) {
    ...
    while(vert != NULL) {
        node = vert->adj_list;
        HAL_printf("%u: [ ", vert->addr);
        while(node != NULL){
            HAL_printf("%u ", node->addr);
            node = node->next;
        }
...
/*
        }else if(vert->addr == config->node_addr) {
            node = vert->adj_list;
            while(node != NULL){
                if(node->addr == target_addr){
                    remove_adj_node(vert, node->addr);
                }
                node = node->next;
            }
        }
        ...
        Removing 4, should walk through every vert and remove 4 from its adj_list
