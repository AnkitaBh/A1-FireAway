/**
  ...
  *                   Modified by karenabruzzo, Ankita 
  * @date           : 14.12.22
**/
...
void populate_schedule(graph_t *graph, schedule_t *sch) {
    mixnet_address *visited = calloc(NUM_NODES, sizeof(mixnet_address));
    mixnet_address *to_visit = calloc(NUM_NODES, sizeof(mixnet_address));
...
void print_schedule(schedule_t *local_sch) {
    ...
    for(int i=0; i<local_sch->total_ts; i++){
        HAL_printf("%u ", local_sch->rx_list[i]);
    }
    HAL_printf("]\n");

    HAL_printf("TX: [ ");
    for(int i=0; i<local_sch->total_ts; i++){
        HAL_printf("%u ", local_sch->tx_list[i]);
    }
    HAL_printf("]\n");

    HAL_printf("schedule receive order: [ ");
    for(int i=0; i<local_sch->total_ts; i++){
        HAL_printf("%u ", local_sch->sch_recv_order[i]);
    }
    HAL_printf("]\n");
}
